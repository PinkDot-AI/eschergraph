...
class PdfToken:
  ...
  def from_poppler_etree(page_number: int, xml_tag: ElementBase, pdf_font: PdfFont):
    ...
    reading_order_no = (
      int(xml_tag.attrib["reading_order_no"])
      if "reading_order_no" in xml_tag.attrib
      else -1
    )
    ...
    return PdfToken(
      page_number, tag_id, content, pdf_font, reading_order_no, bounding_box, token_type
    )
  ...
  def get_same_line_tokens(self, page_tokens: list["PdfToken"]):
    ...
    same_line_tokens = [
      ...
      if top <= each_token.bounding_box.top < (top + height)
      or top < each_token.bounding_box.bottom <= (top + height)
    ]
  ...
  def get_context(self, page_tokens: list["PdfToken"]):
    ...
    on_the_left = [
      each_token
      for each_token in same_line_tokens
      if each_token.bounding_box.right < right
    ]
    on_the_right = [
      each_token
      for each_token in same_line_tokens
      if left < each_token.bounding_box.left
    ]

    if on_the_left:
      self.pdf_token_context.right_of_token_on_the_left = max([
        x.bounding_box.right for x in on_the_left
      ])
      self.pdf_token_context.left_of_token_on_the_left = min([
        x.bounding_box.left for x in on_the_left
      ])

    if on_the_right:
      self.pdf_token_context.left_of_token_on_the_right = min([
        x.bounding_box.left for x in on_the_right
      ])
      self.pdf_token_context.right_of_token_on_the_right = max([
        x.bounding_box.right for x in on_the_right
      ])
