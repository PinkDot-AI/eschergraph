...
class Label(BaseModel):
  ...
  def get_location_discrepancy(self, token_bounding_box: Rectangle):
    coordinates_discrepancy: int = abs(self.left - token_bounding_box.left) + abs(
      self.top - token_bounding_box.top
    )
    size_discrepancy: int = abs(self.height - token_bounding_box.height) + abs(
      self.width - token_bounding_box.width
    )
  ...
  def from_text_elements(text_elements: list[ElementBase]):
    ...
    bottom = max([
      int(x.attrib["top"]) + int(x.attrib["height"]) for x in text_elements
    ])
