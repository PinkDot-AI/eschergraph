...
def test_assign_node(node_matcher_mock: NodeMatcher) -> None:
    node_matcher_mock.reranker.rerank.return_value = [RerankerResult(
        index=1, relevance_score=1.8, text='Patrick Timmermans---mock test'
    )]
    ...
    assert result == "Patrick Timmermans", f"Expected 'Patrick Timmermans' but got '{result}'"
...
def test_collection_node_info(node_matcher_mock: NodeMatcher):
    ...
    build_log = [
        BuildLog(
            ...
            properties=[
                PropertyExt(entity_name='node 1', properties=['node 1 is very cool', 'node 1 has many friends'])
            ],
...
def test_handle_merge(node_matcher_mock: NodeMatcher):
    ...
    with patch('eschergraph.tools.node_matcher.NodeMatcher._get_unique_nodes', return_value=[
        {
            'entities': [
                {
                    'name': 'Lennart Timmermans',
        ...
        with patch('eschergraph.tools.node_matcher.NodeMatcher._build_entity_to_nodes_map', return_value=(
            ...
            with patch('eschergraph.tools.node_matcher.NodeMatcher._process_entities_for_logs', return_value=None) as mock_process_entities_for_logs:
                ...
                mock_process_entities_for_logs.assert_called_once_with(
                    building_logs,
                    mock_build_entity_to_nodes_map.return_value[1]
                )
